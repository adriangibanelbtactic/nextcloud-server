...
class Manager {
	...
	public function getChangelog(string $appId, string $version, ?string $languageCode = null): string|null {
		if ($languageCode === null) {
			$languageCode = $this->l10NFactory->getUserLanguage();
		}
		...
		$changes = $this->retrieveLogEntry($path, $version);
		return $changes;
	}
	 ...
	 */
	protected function retrieveLogEntry(string $path, string $version): string|null {
		$matches = [];
